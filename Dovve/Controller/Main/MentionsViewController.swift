...
class MentionsViewController: UIViewController {
    ...
    var dataModel:[MentionTimeLineModel]?
    ...
    override func viewDidLoad() {
        ...
        MentionTimeLineModel.fetchMentiontimeLine(view:self, max_id: "" , params:"") {(dataModel) in
    ...
    func getDataListArray(_ data:[MentionTimeLineModel]){
    ...
    func scrollViewDidEndDragging(_ scrollView: UIScrollView, willDecelerate decelerate: Bool) {
        ...
        if ((collectionView.contentOffset.y + collectionView.frame.size.height) >= loadMoreFrom){
            if let dataList = dataList {
                ...
                getLastId! -= 1
                MentionTimeLineModel.fetchMentiontimeLine(view:self ,max_id: "max_id=\(getLastId ?? 0)&" , params:"&max_id=\(getLastId ?? 0)") {(dataModel) in
                    self.getDataListArray(dataModel)
                    self.collectionView.reloadData()
                }
            }
...
extension MentionsViewController: SimpleTextPostDelegate, PostWithImagesDelegate, QuotedPostDelegate, QuotedPostWithImageDelegate , PostWithImageAndQuoteDelegate , PostWithImageAndQuotedImageDelegate {
